        ...
        <table class="table">
            @for ($i = 0; $i < count($coches); $i++)
                ...
                <tbody>
                    <tr>
                        <td>
                            <a href="{{ config('app.frontend_url') }}/detalle/{{ $coches[$i]->id }}" class="text-decoration-none text-black">
                                <img src="{{ $coches[$i]->imagen }}" alt="Imagen coche" class="img-fluid w-50" style="max-width:300px"><br>{{ $coches[$i]->marca }} {{ $coches[$i]->modelo }}
                            </a>
                        </td>
